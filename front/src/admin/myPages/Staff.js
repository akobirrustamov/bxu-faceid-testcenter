                    ...
                    <table className="min-w-full table-auto border-collapse border border-gray-300">
                        ...
                        {agents.map((agent, index) => (
                            <tr key={agent.id} className="hover:bg-gray-100">
                                ...
                                <td className="border border-gray-300 px-4 py-2">{agent.agentNumber}</td>
                                <td className="border border-gray-300 px-4 py-2">
                                    <input
                                        type="checkbox"
                                        checked={agent.agent.roles.some(item => item.name === "ROLE_DATA_MANAGER")}
                                        onChange={(e) => handleStatusStaff(agent.agent.id, e.target.checked, "agent", agent.agent.name)}
                                    />
                                </td>
                                <td className="border border-gray-300 px-4 py-2 flex gap-2 justify-center">
                                    <button
                                        className="bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600"
                                        onClick={() => handleEdit(agent.agent, "agent")}
                                    >
                                        Edit
                                    </button>
                                    <button
                                        className="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600"
                                        onClick={() => handleDelete(agent.agent.id, "agent")}
                                    >
                                        Delete
                                    </button>
                                </td>
