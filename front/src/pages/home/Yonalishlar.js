...
function Yonalishlar(props) {
    ...
    useEffect(() => {
        if (educationFields.length > 0) {
            ...
            const forms = [...new Map(educationFields.map(field => [field.educationForm.id, field.educationForm])).values()];
    ...
    const fetchEducationForms = async () => {
        try {
            const response = await ApiCall("/api/v1/education-form/1", "GET", null, null, true);
    ...
    const filteredFields = educationFields.filter((field) => {
        ...
        const matchesTab = activeTab ? field.educationForm?.id === activeTab : false;
        return matchesName && matchesTab;
    });

    return (
        <div className="h-full bg-white border-2">
            <div className="p-4 md:p-6 lg:p-8 xl:p-10 border-2">
                <div className="text-center mb-4">
                    <h2 className="text-2xl sm:text-3xl md:text-4xl xl:text-5xl font-bold">QABUL YO'NALISHLARI</h2>
                ...
                <div className="p-4 sm:p-6 md:p-8 lg:p-10 xl:p-12 pt-0 mx-auto">
                    ...
                    <div className="tabs flex flex-wrap gap-2 mb-4 justify-center">
                        {uniqueEducationForms.map((form) => (
                            ...
                            >
                                {form.educationType.name === "Magistr" ? "Magistratura" : form.name}
                            </span>
                        ))}
                    </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full table-auto border-collapse border border-gray-300 rounded-lg bg-[#EFF9FF]">
                            ...
                            <tr className="bg-[#EFF9FF]">
                                <th className="border border-gray-300 px-2 py-2 text-sm sm:text-base md:text-lg">N%</th>
                                <th className="border border-gray-300 px-2 py-2 text-sm sm:text-base md:text-lg">Yo'nalish nomi</th>
                                <th className="border border-gray-300 px-2 py-2 text-sm sm:text-base md:text-lg">Davomiyligi</th>
                                <th className="border border-gray-300 px-2 py-2 text-sm sm:text-base md:text-lg">Kontrakt summasi</th>
                            </tr>
                            </thead>
                            <tbody>
                            {filteredFields.map((field, index) => (
                                <tr
                                    key={field.id}
                                    className={`${index % 2 === 0 ? 'bg-white' : 'bg-[#EFF9FF]'} hover:bg-gray-50`}
                                >
                                    <td className="border border-gray-300 px-2 py-2 text-sm sm:text-base md:text-lg">{index + 1}</td>
                                    <td className="border border-gray-300 px-2 py-2 text-sm sm:text-base md:text-lg font-semibold">{field.name}</td>
                                    <td className="border border-gray-300 px-2 py-2 text-sm sm:text-base md:text-lg">{field.educationDuration}</td>
                                    <td className="border border-gray-300 px-2 py-2 text-sm sm:text-base md:text-lg">{field.price}</td>
                                </tr>
                            ))}
                            </tbody>
                        </table>
